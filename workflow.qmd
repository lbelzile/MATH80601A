...
We begin with a running example that will be used to illustrate the concepts.
...
posterior <- as.array(postsamp$post_warmup_draws)[,1,]
...
Simulation-based calibration  [@Talts:2020] proceeds with simulation of the following quantities, in the specified order:
...
#| echo: true
#| code-fold: true
# posterior: B x K matrix of posterior draws for the K coefficients
data(smartwatch, package = "hecbayes")
...
loglik_pt[b,] <- dpois(
  x = smartwatch$nviolation,
  lambda = exp(posterior_smartwatch[b,id_alpha] + posterior_smartwatch[b,id_beta]),
  log = TRUE)
postpred[b,] <- rpois(
  n = n,
  lambda = exp(posterior_smartwatch[b,id_alpha] + posterior_smartwatch[b,id_beta]))
...
#| echo: true
#| code-fold: true
is_wgt <- exp(-loglik_pt) # log weights
# importance sampling estimator
ploo <- numeric(n)
for(i in seq_along(y)){
 ploo[i] <- weighted.mean(x = I(postpred[,i] <= y[i]), w = is_wgt[,i])
}
ggplot(data = data.frame(ploo = sort(ploo), punif = ppoints(n)),
...
lw1 <- weights(loo_test1$psis_object)
g1 <- bayesplot::ppc_loo_pit_qq(
  y = smartwatch$nviolation, # n vector of response
  yrep = postpred1,  # B x n matrix of posterior predictive values
  lw = lw1) + # B x n matrix of log weights
 theme_classic()
loo_test2 <- loo::loo(loglik_pt2, save_psis = TRUE)
g2 <- bayesplot::ppc_loo_pit_qq(
  y = smartwatch$nviolation,
  yrep = postpred2, 
  psis_object = loo_test2$psis_object) + 
 theme_classic()
g1 + g2
```
