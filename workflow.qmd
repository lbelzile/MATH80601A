...
We can improve mixing by adding the redundant parameter $\xi_i>0$ [@Gelman.vanDyk:2008] for each observation
\begin{align*}
Y_{i} &\sim \mathsf{Gauss}(\mu + \alpha\xi_i, \sigma^2_i) \\
\xi_i &\sim \mathsf{Gauss}(0, \sigma^2_\xi)
\end{align*}
so that $\alpha_j = \alpha \xi_j$ and $\sigma_\alpha = |\alpha|\sigma_\xi.$ The common parameter $\alpha$ controls both  the variance and the amplitude of the coefficients.
...
#| eval: true
#| echo: false
#| label: fig-eightschool
#| fig-cap: "Average treatment effect with asymptotic 95% confidence intervals for the eight school example."
