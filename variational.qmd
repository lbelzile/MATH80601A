...
We consider Example 10.1.3 from @Bishop:2006 for a variational approximation of a Gaussian distribution with conjugate prior parametrized in terms of precision, with
\begin{align*}
...
\begin{align*}
\tau \mid \boldsymbol{y} &\sim \mathsf{gamma}\left\{ a_0 + (n-1)/2, b_0 + \frac{\sum_{i=1}^n y_i^2 + (n-\tau_0)^{-1}(n\overline{y}+\tau_0)^2}{2}\right\}
\\ \mu \mid \boldsymbol{y}
\end{align*}
...
\begin{align*}
 \log g^{\star}_\mu(\mu) \propto -\frac{\mathsf{E}_{\tau}(\tau)}{2} \left\{ \sum_{i=1}^n (y_i-\mu)^2-\tau_0}{2}(\mu-\mu_0)^2\right\},
\end{align*}
...
\begin{align*}
 \ln g^{\star}_{\tau}(\tau) \propto (n/2 + a_0-1) \log \tau - \tau \left[b_0  + \frac{1}{2} \mathsf{E}_{\mu}\left\{\sum_{i=1}^n (y_i-\mu)^2\right\} + \frac{\tau_0(\mu-\mu_0)^2}{2}\right].
\end{align*}
This is the form as  $p(\tau \mid \mu, \boldsymbol{y}),$ namely a gamma with shape $a_n =n/2 + a_0$ and rate $b_n$ given by the term in the square brackets. It is helpful to rewrite the expected value as
\begin{align*}
...
& \quad - \frac{n+1}{2} \log(2\pi) + \frac{\log(\tau_0) + (n+1)\mathsf{E}_{\tau}(\log \tau)}{2} - \frac{\mathsf{E}_{\tau}(\tau)\tau_0}{2} \left\{\mu_0 - \mathsf{E}_{\mu}(\mu)\}^2 + \mathsf{Va}_{\mu}(\mu)
...
\\& \quad + \frac{1 + \log(2\pi) + \log(n+\tau_0)}{2} + \frac{\mathsf{E}_{\tau}(\log \tau)}{2}
\end{align*}
-->
...
y <- rnorm(n = 20, 150, 20)
 logpost <- function(pars){
    ...
    sum(dnorm(y, mean = mu, sd = 1/sqrt(tau), log = TRUE)) +
      dnorm(mu, sd = 100/sqrt(tau), log = TRUE) + 
...
CAVI_gauss <- function(init = c(mean(y), 1/var(y)),
 ...
 list(elbo = elbo, mean_mu = E_mu, var_mu = var_mu, shape_tau = an, rate = bn)
}
```
