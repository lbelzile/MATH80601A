...
mode_laplace <- optim(
   par = 0.2, 
   fn = function(x){
     -sn::dsn(x, xi = 0, alpha = al, log = TRUE)},
   method = "Brent", 
   lower = -1, 
   upper = 2, hessian = TRUE)
curve(sn::dsn(x, xi = 0, alpha = al), 
     from = -2, 
     to = 4, 
     n = 1001, 
     bty = "n", ylab = "density",
...
dsn <- function(x, loc = 0, scale = 1, alpha){
  ...
  X <- rnorm(10000)
elbo <- function(pars, mu = 0, sigma = 1, alpha = al){
  ms <- pars[1]
  s2 <- exp(pars[2])
...
Consider an approximation with $g$ the density of $\mathsf{Gauss}(m, s^2);$ we obtain the parameters by maximizing the ELBO. The entropy term for a Gaussian approximating density is 
\begin{align*}
...
Given the strong requirements for getting tractable conditionals, coordinate-ascent variational inference is mostly useful in ultra high-dimensional settings where Markov chain Monte Carlo algorithms are not viable. This is the case notable for regression models with the discrete spike-and-slab of @def-spike-slab, models for image processing or topic modelling using latent Dirichlet allocations.

## Skew-approximations

The Gaussian Laplace, variational and expected propagation approximations typically rely on symmetric approximations to the posterior. A symmetric function $g$ about $\boldsymbol{x}_{\text{sym}}$ is one for which the identity $g(\boldsymbol{x}) = g(2\boldsymbol{x}_{\text{sym}}-\boldsymbol{x}_{\text{sym}})$ for any $\boldsymbol{x}$ in the domain.


Symmetric approximations perform poorly when the underlying posterior is skewed, as in the example in the right panel of @fig-convex shows. Recent work of @Pozza.Durante.Szabo:2026 show how one can build a skew-symmetric approximation for the posterior $p(\boldsymbol{\theta} \mid \boldsymbol{y})$ from a symmetric one, say $g(\boldsymbol{\theta})$ at the cost of two additional likelihood and prior evaluations for every parameter vector $\boldsymbol{\theta}$ of interest.

Let $g^{\text{sym}}(\boldsymbol{\theta}; \boldsymbol{\theta}_{\text{sym}})$ be a density function that is symmetric around $\boldsymbol{\theta}_{\text{sym}};$ for a Gaussian or any elliptical distribution, this value $\boldsymbol{\theta}_{\text{sym}}$ would be the location vector $\boldsymbol{\mu}.$ We consider a skewing factor $w(\boldsymbol{\theta};  \boldsymbol{\theta}_{\text{sym}})$ satisfying $w(\boldsymbol{\theta}; \boldsymbol{\theta}_{\text{sym}}) = 1-w(2\boldsymbol{\theta}_{\text{sym}}-\boldsymbol{\theta}; \boldsymbol{\theta}_{\text{sym}})$ for any $\boldsymbol{\theta} \in \boldsymbol{\Theta} \subseteq \mathbb{R}^d.$

The skew-symmetric approximation is obtained by multiplying the symmetric density by a perturbation factor, where 
\begin{align*}
...
In a Bayesian setting with deterministic approximations, we can easily take $g$ to be symmetric, so the only outstanding question is how to define the tilting parameter. Consider a posterior density $p(\boldsymbol{\theta} \mid \boldsymbol{y}) \propto_{\boldsymbol{\theta}} p(\boldsymbol{\theta})p(\boldsymbol{y}\mid \boldsymbol{\theta}).$ Taking as skewing factor 
\begin{align*}
...
\end{align*}
leads to an improvement in fit (in the sense of reducing the Kullback--Leibler divergence with the true distribution) [@Pozza.Durante.Szabo:2026]. The cleverness of this ratio is that, since the unnormalized posterior (prior and likelihood) appear in both the numerator and the denominator, this amounts to using the posterior density in the skewing factor definition, as the marginal likelihood would cancel out. If the posterior is zero at some value of $\boldsymbol{\theta},$ which is outside the support, then we define $w(\boldsymbol{\theta};\boldsymbol{\theta}_{\text{sym}})=0.5$ to satisfy the properties of $w(\cdot;\cdot).$

While the above gives an approximation to the density, it is sometimes useful to generate samples from the skewed approximating density. Proposition 1 of @Pozza.Durante.Szabo:2026 show that we can generate from the skewed approximation by first sampling a candidate $\boldsymbol{\omega}_0$ from the model with density $g^{\text{sym}}(\cdot; \boldsymbol{\theta}_{\text{sym}})$ and $U \sim \mathsf{unif}(0,1),$ then setting
\begin{align*}
\boldsymbol{\theta}_0 = 
\begin{cases} 
\boldsymbol{\omega}_0, & 
