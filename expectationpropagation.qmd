...
\end{align*}
The term $\psi(\theta_{((i)})$ is constant, so if we plug-in this inside the exponential, we obtain
\begin{align*}
...
\end{align*}
If we apply integration by parts (Stein's lemma, see @prp-stein) using the fact that the integral is with respect to a standard Gaussian density $\phi_p(\boldsymbol{z}),$ we can rewrite the second term as
\begin{align*}
...
Given the mean and variance of the cavity, denoted $\mu_{-j}$ and $\boldsymbol{\Sigma}_{-j},$ and with $s_j = \mathbf{x}_j\boldsymbol{\Sigma}_{-j}\mathbf{x}_j^\top$, the updated mean and variance are
\begin{align*}
...
\end{align*}
where $Z_j$ has distribution proportional to $L_j(z) \times \phi(z; \mathbf{x}_j\boldsymbol{\mu}_j, \mathbf{x}_j\boldsymbol{\Sigma}_{-j}\mathbf{x}_j^\top),$ where $\phi(\cdot; \mu, \sigma^2)$ denotes the density of a Gaussian random variable. The updates to the parameters are found in page 23 of @Cseke.Heskes:2011.
