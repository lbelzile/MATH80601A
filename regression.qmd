...
where $\boldsymbol{\beta}^{(m)} \mid M_m,$ is assigned a Gaussian prior $\. $\boldsymbol{\Sigma}$  are given suitable priors. We write $|M|$ for the cardinality of the set of non-zero coefficients $\boldsymbol{\beta}$ in model $M.$
