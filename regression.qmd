...
where $\boldsymbol{\beta}^{(m)} \mid M_m,$ is assigned a Gaussian prior $\. $\boldsymbol{\Sigma}$  are given suitable priors. We write $|M_m|$ for the cardinality of the set of non-zero coefficients $\boldsymbol{\beta}$ in model $M_m.$

We can consider a joint update of the regression parameters $\boldsymbol{\beta}, M$ by sampling from their joint distribution via $p(\boldsymbol{\beta} \mid M, \cdots) p(M \mid \cdots).$ Write $\boldsymbol{\theta}$ for all parameters other than the response, model and vector of coefficients. The update for the coefficients is as usual, while for the model we get the conditional Bayes factor,
\begin{align*}
...
\end{align*}
We can marginalize thus marginalize out over $\boldsymbol{\beta}.$ We can show (demonstration omitted) that
\begin{align*}
