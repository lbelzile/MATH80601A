...
\end{align*}
where $p(\sigma^2 \mid y) \sim \mathsf{InvGamma}(\alpha^*, \beta^*)$ and 
\begin{align*}
...
If we run a MCMC algorithm to sample from a mixture models, the likelihood is invariant to permutation of the group labels, leading to identifiability issues when running multiple Markov chains.
...
\begin{align*}
f_i(x) = p_i f_{1}(x_i; \mu_1, \tau_1^{-1}) + (1-p_i)f_{2}(x_i; \mu_2, \tau_2^{-1}).
\end{align*}
...
for(b in 1:B){
  d1 <- dnorm(y, mean = mu[1], sd = 1/sqrt(prec[1]))
  d2 <- dnorm(y, mean = mu[2], sd = 1/sqrt(prec[2]))
  # Data augmentation: group labels
  group <- rbinom(n = n, size = rep(1, n), prob = p*d1/(p*d1 + (1-p)*d2))
  # Step 2: update probability of cluster
  p <- rbeta(n = 1, shape1 = sum(group) + a1, n - sum(group) + a2)
  for(j in 1:2){
    yg <- y[group == (j-1L)]
    ng <- length(yg)
    prec_mu <- ng*prec[j] + d
...
\end{align*}
This yields an intrinsic Gaussian Markov random field with a circulant precision matrix $\tau\mathbf{Q}=\tau\mathbf{GG}$ of rank $T-1,$ where
\begin{align*}
\mathbf{G} = 
\begin{pmatrix} 
2 & -1 & 0 & 0 & \cdots & -1\\
...
We can perform data augmentation by imputing Gaussian variables, say $x_{1,1}, x_{i,2}, \ldots, x_{366,2}$ following @exm-probit-regression from truncated Gaussian, where $x_{t,i} = \beta_t + \varepsilon_{t,i}$ and $\varepsilon_{t,i} \sim \mathsf{Gauss}(0,1)$ are independent standard Gaussian and 
\begin{align*}
x_{t,i} \mid  y_{t,i}, \beta_t \sim 
\begin{cases}
\mathsf{trunc. Gauss}(\beta_t, 1, 0, \infty) & y_{t,i} = 0 \\
\mathsf{trunc. Gauss}(\beta_t, 1, -\infty, 0) & y_{t,i} =1 
\end{cases}
\end{align*}
The posterior is proportional to
\begin{align*}
p(\boldsymbol{\beta} \mid \tau)p(\tau)\prod_{t=1}^{T}\prod_{i=1}^{n_t}p(y_{t,i} \mid x_{t,i}) p(x_{t,i} \mid \beta_t)
\end{align*}
and once we have imputed the Gaussian latent vectors, we can work directly with the values of $x_t = \sum_{i=1}^{n_t} x_{i,t}.$ The posterior has the form
\begin{align*}
p(\boldsymbol{\beta}, \tau) \propto \tau^{(n-1)/2}\exp \left( - \frac{\tau}{2} \boldsymbol{\beta}^\top \mathbf{Q} \boldsymbol{\beta}\right)\exp\left\{ - \frac{1}{2} (\boldsymbol{x} - \boldsymbol{\beta})^\top \mathrm{diag}(\boldsymbol{n})(\boldsymbol{x} - \boldsymbol{\beta})\right\} \tau^{a-1}\exp(-\tau b)
\end{align*}
where $\boldsymbol{x} = (x_1, \ldots, x_T).$
Completing the quadratic form shows that 
\begin{align*}
\boldsymbol{\beta} \mid \boldsymbol{x}, \tau &\sim \mathsf{Gauss}_T\left[\left\{\tau \mathbf{Q} + \mathrm{diag}(\boldsymbol{n})\right\}^{-1} \mathrm{diag}(\boldsymbol{n}) \boldsymbol{x}, \left\{\tau \mathbf{Q} + \mathrm{diag}(\boldsymbol{n})\right\}^{-1}\right]\\
\tau \mid \boldsymbol{\beta}, \boldsymbol{x} & \sim \mathsf{gamma}\left( \frac{n-1}{2} + a, \frac{\boldsymbol{beta}^\top \mathbf{Q}\boldsymbol{\beta}{2} + b \right)
\end{align*}
...
for(b in seq_len(B)){
  if(b %% 100L == 0L){
    print(paste(b, 'iterations completed'));
  }
# Step 1: data augmentation
x <- TruncatedNormal::rtnorm(
  n = 1,
  lb = ifelse(y == 0, -Inf, -beta),
  ub = ifelse(y == 0, -beta, Inf)) + beta[day]
tx <- aggregate(x = x, by = list(day), FUN = sum)$x
x_s[b,] <- tx
# Simulate random effects in block
beta <- beta_s[b,] <- rGaussQ(
  n = 1,
  b = Tokyo$n * tx,
  Q = tau * Q + N)
# st <- sample.int(nt, 1)
## Update individually looping over
...
#' Simulate from Gaussian random field 
#' 
#' Given a Gaussian random field with precision matrix \eqn{Q} and mean \eqn{Q^{-1}b}, simulate a
