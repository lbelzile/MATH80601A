...
The posterior distribution combines two ingredients: the likelihood and the prior. If the former is a standard ingredient of any likelihood-based modelling, prior specification is more delicate. The purpose of this chapter is to consider different ways of constructing prior functions, although these are generally chosen for convenience. Priors are somewhat controversial among frequentist statisticians because they can impact the outcome results, and different prior elicitation yields potentially different conclusions.

While the sole requirement for inference is that the posterior is proper, the most common choice of prior function is to take a density function of random variables. Using the latter, we can use moment matching to get sensible values or tune via trial-and-error using the prior predictive draws to assess the implausibility of the prior outcomes. One challenge is that even if we have some prior information (e.g., we can obtain sensible prior values for the mean, quantiles or variance of the parameter of interest), these summary statistics will not typically be enough to fully characterize the prior as many different functions or distributions could encode the same information.

The posterior is a compromise between the prior and the likelihood. With independent data, the prior has weight $\mathrm{O}(1)$ and the likelihood $\mathrm{O}(n)$, so the effect of the prior typically becomes negligible in large samples, but in multi-level models, not all observations need to be informative for parameters in the hierarchy and careful prior elicitation becomes crucial.

We can assess the robustness of the prior specification through a sensitivity analysis by comparing the outcomes of the posterior for different priors or different values of the hyperparameters, which are the parameters of the prior.
...
Consider instead a Bayesian analysis with conjugate prior: we model separately the rates of each group (question or not). Suppose we think apriori that the click-rate is on average 1%, with a standard deviation of 2%, regardless of whether the title asks a question or not. If we adopt a $\mathsf{Gamma}(\alpha, \beta)$ prior, we can transfer this prior specification using moment matching into suitable values for the hyperparameters, by taking a rate of $\beta = \mathsf{E}_0(\lambda_j)/ \mathsf{Var}_0(\lambda_j)=0.01/0.02^2=25$ and a shape of $\alpha=\mathsf{E}_0(\lambda_j)\times\beta=0.01 \times 25=0.25$ ($j=1, 2$). If $\lambda_{j}$ is the average rate for each factor level (yes and no), then $\mathsf{E}(Y_{ij}/n_{ij}) = \lambda_j$ so the log likelihood is proportional, as a function of $\lambda_1$ and $\lambda_2,$ to
\begin{align*}
...
To get an approximation to the posterior mean of the ratio $\lambda_1/\lambda_2,$ it suffices to draw independent observations from their respective posterior, compute the ratio and take the sample mean of those draws: the later is nearly symmetric.
...
We call a prior function *proper* if it's integral is finite over the parameter space; such prior function automatically leads to a valid posterior. A prior over $\boldsymbol{\Theta}$ is **improper** if $\int_{\boldsymbol{\Theta}} p(\boldsymbol{\theta}) \mathrm{d} \boldsymbol{\theta} = \infty.$
...
* Priors are distributions for the parameters. In multi-parameter models, they can be specified through a joint distribution or assumed independent a priori (which does not translate into independence a posteriori).
...
* Priors that restrict the domain of $\boldsymbol{\theta}$ will also restrict the posterior. These are useful to avoid regions of the parameter space that are implausible or impossible.
