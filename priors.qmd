...
- define and distinguish between flat, vague, weak and informative priors.
...
Consider the binomial distribution $\mathsf{Bin}(1, \theta)$ with density $f(y; \theta) \propto  \theta^y(1-\theta)^{1-y}\mathbf{1}_{\theta \in [0,1]}.$ The negative of the second derivative of the log likelihood with respect to $\theta$ is $$\jmath(\theta) = - \partial^2 \ell(\theta; y) / \partial \theta^2 = y/\theta^2 + (1-y)/(1-\theta)^2$$ and since $\mathsf{E}(Y)=\theta,$ the Fisher information is $$\imath(\theta) = \mathsf{E}\{\jmath(\theta)\}=1/\theta + 1/(1-\theta) = 1/\{\theta(1-\theta)\}$$
