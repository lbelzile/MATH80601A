...
\intertext{so with $\xi_j=1/\tau_j$, we have}
...
\end{align*}
This yields a Laplace penalty on the regression coefficients, with lower values of $\lambda$ yielding more shrinkage. Note that, in the Bayesian LASSO, none of the posterior draws of $\boldsymbol{\beta}$ are exactly zero.
...
With the augmented data, the conditional distribution of $Z_i \mid X_i, \boldsymbol{\omega}, \boldsymbol{\theta} \sim \mathsf{Mult}(1, \boldsymbol{\gamma}_{ik})$ where $\gamma_{ik} = \omega_k f_k(X_i\boldsymbol{\theta}_k)/\big\{\sum_{j=1}^K f_j(X_i\boldsymbol{\theta}_k)\big\}$.
