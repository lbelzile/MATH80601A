...
The simpler Metropolis-adjusted Langevin algorithm is equivalent to using a Gaussian random walk where the proposal has mean $\boldsymbol{\theta}_{t-1} + \mathbf{A}\eta \nabla \ell(\boldsymbol{\theta}_{t-1})$ and variance $\tau^2\mathbf{A}$, for some mass matrix $\mathbf{A}$ and a damping factor $\eta$. Taking $\mathbf{A}$ as the identity matrix, which assumes the parameters are isotropic (same variance, uncorrelated) is the default choice although seldom far from optimal.
...
\end{align*}
We can approximate this quantity pointwise by averaging the log posterior density of each sample observation over the posterior samples via Monte Carlo. The higher the value of the predictive density $\mathsf{LPPD}$, the better the fit. 
