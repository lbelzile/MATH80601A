...
\end{align*}
@Wakefield.Gelfand.Smith:1991 propose using $r=1/2$ and relocating the mode of $f$ to the origin increase the acceptance rate. Northrop proposes to use a Box--Cox transformation [@Box.Cox:1964] together with a rotation in the software @rust to improve the acceptance rate. The bounding box may exist only for certain values of $r$; see the vignettes for more examples.
...
Using the law of iterated expectation and variance, it the process is weakly stationary, then $\mathsf{E}_{Y_{t}}(Y_t)=\mathsf{E}_{Y_{t-1}}(Y_{t-1})$
\begin{align*}
