...
The ratio-of-uniform method [@Kinderman.Monahan:1977; @Wakefield.Gelfand.Smith:1991] is a variant of accept-reject used to draw samples from a unnormalized density $f(\boldsymbol{\theta})$ for $\boldsymbol{\theta} \in \boldsymbol{\Theta} \subseteq \mathbb{R}^d.$ For some $r \geq 0,$ consider the set \begin{align*}
...
Consider estimation of the probability that a standard Gaussian random variable exceeds $a=4,$ which is $p=1-\Phi(a).$ We can use standard numerical approximations to the distribution function implemented in any software package, which shows this probability is roughly $1/`r as.integer(round(1/pnorm(4, lower.tail = FALSE),0))`.$ 
...
then $\{Y_t\}$ is ergodic for the mean and $\overline{Y} \stackrel{\mathrm{p}}{\to} \mu$. In other words, the ergodic theorem is a law of large numbers for stochastic processes that allows for serial dependence between observations, provided the dependence is not too large.
...
Intuitively, a sample of correlated observations carries less information than an independent sample of draws. Suppose that the **unconditional variance** is $\tau^2$. If we want to compute sample averages $\overline{Y}_T=(Y_1+ \cdots + Y_T)/T,$ the variance will be 
\begin{align*}
...
In the independent case, the covariance is zero so we get the sum of variances. If the process is stationary, the correlation depends only on the lag $k$ and not on $t$ and the variance is constant, so 
$$\mathsf{Cov}(Y_{t-k}, Y_{t})=$\tau^2$\mathsf{Cor}(Y_{t-k}, Y_{t})=$\tau^2$\rho_k,$$ say. 
Thus,
\begin{align*}
\mathsf{Va}(\overline{Y}_T) = \frac{$\tau^2$}{T} \left\{ 1 + 2\sum_{s=1}^{T-1} \frac{(T-s)}{T}\rho_s\right\}.
\end{align*} 
 Under technical conditions[^mcmc-4], a central limit theorem applies and we get an asymptotic variance for the mean of \begin{align*}
...
```{dot}
//| label: fig-P1
//| fig-cap: "Graphical representation of the transition matrix $\\mathbf{P}_1$."
//| fig-align: 'center'
digraph G {
 A [label="State 1"];
 B [label="State 2"];
...
@fig-discrete-markov-chain shows the path of the random walk driven by $\mathbf{P}_3$ and the empirical proportion of the time spent in each state, as time progress. Since the Markov chain has a unique stationary distribution, we expect the sample proportions to converge to the stationary distribution proportions.

```{r}
#| label: fig-discrete-markov-chain
#| eval: true
#| echo: false
#| fig-cap: "Discrete Markov chain on integers from 1 to 5, with transition matrix $\\mathbf{P}_3,$ with traceplot of 1000 first iterations (left) and running mean plots of sample proportion of each state visited per 100 iterations (right)."
