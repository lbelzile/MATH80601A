...
\end{align*}
for some positive radius parameter $r \geq 0$. The measure of the set $\mathcal{B}(r)=c^{-1}(1+rd)^{-1}.$ By the @thm-fundamental-simulation, $(u_1 / u_0^r, \ldots, u_d / u_0^r)$ is drawn from the renormalized density $cf(x)$. The challenge lies in simulating $\boldsymbol{u}$ uniformly over the bounding box $\mathbb{B}$, but we can use accept-reject if the later is enclosed in a bounding box, keeping only samples that satisfy the constraints. If, over $\\mathcal{S}$, $f(\boldsymbol{x})$ and $x_i^{r d +1} f(\boldsymbol{x})^r$ for $i = 1, \ldots, d$ are bounded then we can enclose $\mathcal{B}(r)$ within the $(d+1)$-dimensional bounding box $\mathbb{B}=\{a_j(r) \leq u_i \leq b_j(r);  j = 0, \ldots, d \}$, with $a_0(r)=0$. The parameters of the bounding box are  
\begin{align*}
...
For the ratio-of-uniform method to work, we need the density $f(\boldsymbol{x})$ to be bounded, and the marginal moments of order $rd+1$, meaning 
\begin{align*}
\mathsf{E}(X_i^{rd+1}) = \int_{\mathcal{S}} x_j^{rd+1} f(\boldsymbol{x}) \mathrm{d}x_1 \cdots \mathrm{d} x_d< \infty
\end{align*}
@Wakefield.Gelfand.Smith:1991 propose using $r=1/2$ and relocating the mode of $f$ to the origin increase the acceptance rate. Northrop proposes to use a Box--Cox transformation [@Box.Cox:1964] together with a rotation in the software @rust to improve the acceptance rate. The bounding box may exist only for certain values of $r$; see ee [the `rust` package vignette](https://paulnorthrop.github.io/rust/articles/rust-a-vignette.html) for technical details and examples.
...
A stochastic (i.e., random) process is (strongly) stationary if the distribution of $\{X_1, \ldots, X_t\}$ is the same as that of $\{X_{n+1}, \ldots X_{t+n}\}$ for any value of $n$ and given $t.$
...
Consider a Gaussian $\mathsf{AR}(1)$ model with $Y_t \sim \mathsf{Gauss}\{\mu + \phi(Y_{t-1} - \mu), \sigma^2\}$.


Using the law of iterated expectation and variance, if the process is weakly stationary, then $\mathsf{E}_{Y_{t}}(Y_t)=\mathsf{E}_{Y_{t-1}}(Y_{t-1})$
\begin{align*}
...
\end{align*} 
Denote the lag-$k$ autocorrelation $\mathsf{Cor}(Y_{t}, Y_{t+k})=\gamma_k.$ Under technical conditions[^mcmc-4], a central limit theorem applies and we get an asymptotic variance for the mean of \begin{align*}
