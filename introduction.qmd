...
We begin with a characterization of random vectors and their marginal, conditional and joint distributions. A good reference for this material is Chapter 3 of @McNeil.Frey.Embrechts:2005.
...
A univariate distribution is an exponential family if it's density or mass function can be written for all $y \in \mathbb{R}$ as
\begin{align*}
...
Denote by $f_X$ and $f_Y$ denotes the marginal density of $X$ and $Y$, respectively, $f_{X \mid Y}$ the conditional of $X$ given $Y$ and $f_{X,Y}$ the joint density. Bayes' theorem states that
\begin{align*}
f_{X,Y}(x,y) = \frac{f_{Y}(y \mid X = x)f_X(x)}{f_Y(y)}
\end{align*}
