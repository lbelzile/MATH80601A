...
pmf_post <- dbetabinom(x = x, 
                       ...
                       shape1 = alpha + k, 
                       shape2 = beta + size - k)
pmf_lik <- dbinom(x = x, size = size, prob = k/size)
...
#| warning: false
#| fig-align: 'center'
#| fig-cap: "Posterior density (left) and losses functionsfor the 0.99 value-at-risk for the Danish fire insurance data. The vertical lines denote point estimates of the quantiles that minimize the loss functions."
VaR_post <- with(post_samp,   # data frame of posterior draws
            revdbayes::qgp(   # with columns 'probexc', 'scale', 'shape'
  p = 0.01/probexc, 
  loc = 10, 
