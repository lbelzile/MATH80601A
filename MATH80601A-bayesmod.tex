...
and were last compiled on Monday, March 10 2025.
...
\begin{tcolorbox}[enhanced jigsaw, breakable, rightrule=.15mm, bottomtitle=1mm, coltitle=black, toprule=.15mm, colframe=quarto-callout-important-color-frame, leftrule=.75mm, opacityback=0, toptitle=1mm, arc=.35mm, titlerule=0mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{\textbf{Learning objectives}:}, left=2mm, opacitybacktitle=0.6, colback=white, colbacktitle=quarto-callout-important-color!10!white]
...
\begin{tcolorbox}[enhanced jigsaw, breakable, rightrule=.15mm, bottomtitle=1mm, coltitle=black, toprule=.15mm, colframe=quarto-callout-important-color-frame, leftrule=.75mm, opacityback=0, toptitle=1mm, arc=.35mm, titlerule=0mm, bottomrule=.15mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{\textbf{Summary}}, left=2mm, opacitybacktitle=0.6, colback=white, colbacktitle=quarto-callout-important-color!10!white]
...
proper (\citeproc{ref-Green:2001}{Peter J. Green 2001, 55}).
\end{remark}
...
(\citeproc{ref-Green:1995}{Peter J. Green 1995}), an extension that
allows for arbitrary measures and through this varying dimensions, which
occurs not only with variable selection, but also changepoint analysis
and mixture models with varying number of components
(\citeproc{ref-Green:2001}{Peter J. Green 2001}). Reversible jump
requires a form of data augmentation to have dimension-balancing and
defining different types of moves for jumping between dimensions. These
are integrated in the Metropolis--Hastings step through a Jacobian term
added to \(R\). with different types of moves giving termed padding
andone to pad the dimensions to match and define. In regression models,
we will consider moves that adds or removes one parameter/regressor at a
time.
...
vector \begin{align*}
\boldsymbol{Y} \mid M_m, \boldsymbol{\beta}, \sim \mathsf{Gauss}(\mathbf{X}^{(m)}\boldsymbol{\beta}^{(m)}, \boldsymbol{\Sigma})                                                                                  \end{align*}
where \(\boldsymbol{\beta}^{(m)} \mid M_m,\) is assigned a Gaussian
prior \$. \(\boldsymbol{\Sigma}\) are given suitable priors. We write
\(|M_m|\) for the cardinality of the set of non-zero coefficients
\(\boldsymbol{\beta}\) in model \(M_m.\)

We can consider a joint update of the regression parameters
\(\boldsymbol{\beta}, M\) by sampling from their joint distribution via
\(p(\boldsymbol{\beta} \mid M, \cdots) p(M \mid \cdots).\) Write
\(\boldsymbol{\theta}\) for all parameters other than the response,
model and vector of coefficients. The update for the coefficients is as
usual, while for the model we get the conditional Bayes factor,
\begin{align*}
...
\end{align*} We can marginalize thus marginalize out over
\(\boldsymbol{\beta}.\) We can show (demonstration omitted) that
\begin{align*}
