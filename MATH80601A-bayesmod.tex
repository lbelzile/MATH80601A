...
and were last compiled on lundi, janvier 06 2025.
...
\begin{align*}
f(y; p)=\exp \left\{ \log \Gamma(n+1) - \log \left(\frac{\Gamma(y+1)}{\Gamma(n-y+1)}\right) + y \{\log p - log(1-p)\} + n \log(1-p)\right\}.
...
\(g^{-1}.\) Then, if \(\boldsymbol{Y}=\boldsymbol{g}(\boldsymbol{X}),\)
\begin{align*}
...
\(X_j \sim \mathsf{Gauss}(0, \sigma^2)\) for \(j=1, \ldots, d,\) with
joint density function \begin{align*}
...
\phantomsection\label{rmk-gaussian-dist-property}
\section{Simulation of Gaussian
vectors}\label{simulation-of-gaussian-vectors}
...
Taking \(h(\boldsymbol{y}) = \boldsymbol{y}\) yields the
...
If a random variable \(X\) has distribution function \(F\), then we can
define it's \textbf{generalized inverse} \begin{align*}
F^{-1}(u) = \inf\{x: f(x) \geq u\}
\end{align*} and if \(G\) is continuous, then
\(F(X) \sim \mathsf{unif}(0,1)\). We can thus simulate data using the
quantile function \(F^{-1}(U),\), with \(U \sim \mathsf{unif}(0,1)\). T
...
of \begin{align*}
\widetilde{E}\{g(X)\} =\frac{B^{-1} \sum_{b=1}^B w_b g(x_b) }{B^{-1}\sum_{b=1}^B w_b}.
