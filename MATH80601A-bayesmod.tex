...
vector after the updates. The Gibbs sampling can be viewed as a special
case of Metropolis--Hastings where the proposal distribution \(q\) is
...
independent, are truncated Gaussian \begin{align*}
...
\(\nu=\{\sigma/(y_i-\mu)^2\}^{1/2}\) and \(\lambda=\sigma^{-1}\).
...
where
\(\gamma_{ik} = \omega_k f_k(X_i\boldsymbol{\theta}_k)/\big\{\sum_{j=1}^K f_j(X_i\boldsymbol{\theta}_k)\big\}\).
