...
and were last compiled on Monday, October 02 2023.
...
\(\mathrm{I}(x<1)\) (left), \(x\) (middle) and \(1/x\) (right) for a
Gamma distribution with shape 0.5 and rate 2, as a function of the Monte
Carlo sample size.}
...
proposal with density \(p(\cdot)\), where \(q(\cdot)\) is such that
...
\item
  Compute the ratio
  \(R \gets C^{-1} p(\boldsymbol{\theta}^{\star})/ q(\boldsymbol{\theta}^{\star})\).
\item
  If \(R \leq U\), return \(\boldsymbol{\theta}\), else return to step
  1.
\end{enumerate}
...
is \(C=\{\Phi(\beta) - \Phi(\alpha)\}\). We thus simply simulate points
from the Gaussian and accept any that falls within the bounds.
...
the autoregressive process, \(\mu\) the global mean and
\(\varepsilon_t\) is an iid innovation with mean zero and variance
\(\sigma^2\). If \(|\phi| < 1\), the process is stationary, and the
variance does not increase with \(t\). If innovations are Gaussian, then
we have
...
\begin{proposition}[]\label{prp-variance-clt}
...
A visual diagnostic of the time dependence of a Markov chain is the
correlogram, which shows the empirical lag-\(k\) autocorrelation
estimates against lag.
...
\item
  With probability \(\max\{R, 1\}\), accept the proposal and set
...
resulting draws should be drawn from the prior
(\protect\hyperlink{ref-Green:1994}{Green 1995}).
...
\leavevmode\vadjust pre{\hypertarget{prp-visual-diagnostics}{}}%
\begin{proposition}[Visual diagnostics for Markov
chains]\label{prp-visual-diagnostics}
...
the posterior mode, then we can suspect that it either hasn't reached
it's stationary distribution (likely due to poor starting values). In
such cases, we need to disregard the dubious draws from the chain by
