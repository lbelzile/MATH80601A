...
\end{align*} We use indices \(1:d = \{1, \ldots, d\}\) to explicit which
components of \(\boldsymbol{X}\) are under consideration.
...
\(\boldsymbol{X}\) \((1 \leq k \leq d)\) is \begin{align*}
...
Let
\(\boldsymbol{X}_{1:d} = (\boldsymbol{X}_{1:k}^\top, \boldsymbol{X}_{(k+1):d}^\top)^\top\)
be a \(d\)-dimensional random vector with absolutely continuous margins
and joint density \(f_{1:d}\), or else a discrete distribution with mass
function \(f_{1:d}\). The conditional distribution function of
\(\boldsymbol{X}_{1:k}\) given \(\boldsymbol{X}_{(k+1):d}\), is defined
implicitly from the relationship \begin{align*}
...
mass and \(f_{(k+1):d}(\boldsymbol{x}_{(k+1):d})>0\); it is undefined
otherwise.
...
Let
\(\boldsymbol{X}_{1:d} = (\boldsymbol{X}_{1:k}^\top, \boldsymbol{X}_{(k+1):d}^\top)^\top\)
be a \(d\)-dimensional random vector. We say that
\(\boldsymbol{X}_{1:k}\) and \(\boldsymbol{X}_{(k+1):d}\) are
independent if their joint distribution function factorizes as
\begin{align*}
F_{1:d}(\boldsymbol{x}_{1:k}, \boldsymbol{x}_{(k+1):d}) = F_{1:k}(\boldsymbol{x}_{1:k})F_{(k+1):d}(\boldsymbol{x}_{(k+1):d})
\end{align*} for any value of \(\boldsymbol{x}_{1:k}\),
\(\boldsymbol{x}_{(k+1):d}\). It follows from the definition of joint
density that, should the latter exists, it also factorizes as
\begin{align*}
f_{1:d}(\boldsymbol{x}_{1:k}, \boldsymbol{x}_{(k+1):d}) = f_{1:k}(\boldsymbol{x}_{1:k})f_{(k+1):d}(\boldsymbol{x}_{(k+1):d}).
\end{align*}

If two subvectors \(\boldsymbol{X}_{1:k}\) and
\(\boldsymbol{X}_{(k+1):d}\) are independent, then the conditional
density \(f_{1:k \mid (k+1):d}\) equals the marginal \(f_{1:k}\).
...
we recover the \textbf{exponential distribution}. The case
\(\mathsf{gamma}(\nu/2, 1/2)\) corresponds to the chi-square
distribution \(\chi^2_\nu\).
...
individual observations, whereas \(\boldsymbol{Q}^{-1}\), often denoted
\(\boldsymbol{\Sigma}\) is the \(d \times d\) covariance matrix of
\(\boldsymbol{Y}\) and \(\boldsymbol{Q}\), the canonical parameter, is
the precision matrix.

In the univariate case, the density of
\(\mathsf{normal}(\mu, \sigma^2)\) reduces to \begin{align*}
...
The Gaussian distribution is an elliptical distribution which is
symmetric around it's mode, and it is a location-scale family: if
\(\boldsymbol{L} = \mathrm{chol}(\boldsymbol{Q})\), meaning
\(\boldsymbol{Q}=\boldsymbol{LL}^\top\) for some lower triangular matrix
\(\boldsymbol{L}\), then
\[\boldsymbol{L}^\top(\boldsymbol{Y}-\boldsymbol{\mu}) \sim \mathsf{normal}_d(\boldsymbol{0}_d, \mathbf{I}_d).\]
Conversely, we can use the Cholesky root to sample multivariate Gaussian
vectors by forward solving

The canonical parameter for the scale is the \textbf{precision matrix}
\(\boldsymbol{Q} = \boldsymbol{\Sigma}^{-1},\) the inverse covariance.

Quadratic forms of Gaussians
\((\boldsymbol{Y}-\boldsymbol{\mu})^\top\boldsymbol{\Sigma}^{-1}(\boldsymbol{Y}-\boldsymbol{\mu})\sim \chi^2(n).\)

:::

\begin{proposition}[Partitionning of covariance
...
\(1\leq k < d.\) Show that the conditional distribution \begin{align*}
...
\(\boldsymbol{Y}=(\boldsymbol{Y}_1, \boldsymbol{Y}_2)\). The conditional
density as a function of \(\boldsymbol{y}_1\) is given, up to
proportionality, by \begin{align*}
...
An alternative derivation of the result is given below. Suppose without
loss of generality that \(\boldsymbol{\mu}=\boldsymbol{0}_n\). One can
write the joint density as the product of the marginal and conditional
densities \begin{align*}
...
\end{align*} Completing the square in \(\boldsymbol{y}_1\) in the
exponential gives \begin{align}
...
\boldsymbol{Q}_{11}^{-1}\boldsymbol{Q}_{12}\boldsymbol{y}_2) + \boldsymbol{y}_2^\top \boldsymbol{\Sigma}_{22}^{-1}\boldsymbol{y}_2 \tag{S1} \label{eq:square}
 \end{align} and we can interpret the first term on the right hand side
as a function of \(\boldsymbol{y}_1\) given \(\boldsymbol{y}_2\) and the
second as \(\boldsymbol{y}_2\) alone. By using the transformation matrix
\(\mathbf{A}=(\mathbf{I}_k, \mathbf{O}_{k, n-k})^\top\), we conclude
that
\(\boldsymbol{Y}_2 \sim \mathsf{normal}_{n-k}(\boldsymbol{\mu}_2, \boldsymbol{\Sigma}_{22})\).
The marginal distribution can also be obtained by integrating out
\(\boldsymbol{y}_1\) in
\(f_{\boldsymbol{Y}}(\boldsymbol{y}_1, \boldsymbol{y}_2)\). Thus, the
conditional density is the ratio \begin{align*}
...
\end{align*} making use of the identity
\(|\boldsymbol{Q}| = |\boldsymbol{Q}_{11}||\boldsymbol{\Sigma}_{22}|^{-1}.\)
...
symmetric around \(\boldsymbol{0}_d\) and unimodal. As
...
\includegraphics{introduction_files/figure-pdf/fig-monte-carlo-path-1.pdf}
...
\includegraphics{introduction_files/figure-pdf/fig-acceptreject-1.pdf}
...
is an iid innovation with mean zer o and variance \(\sigma^2\). If
...
\includegraphics{introduction_files/figure-pdf/fig-ar1-variance-1.pdf}
...
\includegraphics{introduction_files/figure-pdf/fig-mcmc-batchmean-1.pdf}
...
\includegraphics{introduction_files/figure-pdf/fig-discrete-markov-chain-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-betabinom-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-binom-massvslik-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-sequential-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-betabinompostpred-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-central-moments-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-danish-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-danish-rou-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-lossfn-1.pdf}
...
\includegraphics{bayesics_files/figure-pdf/fig-credible-intervals-1.pdf}
...
\(\beta \sim \mathsf{Norm}(0, 100)\), weakly informative priors provide
little constraints \(\beta \sim \mathsf{Norm}(0, 10)\), and informative
...
\includegraphics{mcmc_files/figure-pdf/fig-lasso-traceplot-1.pdf}
...
\includegraphics{mcmc_files/figure-pdf/fig-waic-blasso-1.pdf}
