...
and were last compiled on Thursday, February 27 2025.
...
\beta_j \mid \sigma, \tau_j &\sim \mathsf{Gauss}(0, \sigma\tau_j)\\
...
\boldsymbol{\beta} \mid \sigma^2, \boldsymbol{\tau}, \boldsymbol{y} &\sim \mathsf{Gauss}_p\left\{\left(\boldsymbol{X}^\top\boldsymbol{X} + \mathbf{D}^{-1}_{\tau}\right)^{-1} \mathbf{X}\widetilde{\boldsymbol{y}}, \sigma^2\left(\boldsymbol{X}^\top\boldsymbol{X} + \mathbf{D}^{-1}_{\tau}\right)^{-1}\right\}\\
...
\tau_j^{-1} \mid \boldsymbol{\beta}, \sigma^2,\boldsymbol{y} &\sim \mathsf{Wald} \left( \frac{\lambda\sigma}{|\beta_j|}, \lambda^2\right)
\end{align*} where the last three conditional follow from marginalizing
out \(\mu.\)
...
p(\boldsymbol{\beta}, \tau) \propto \tau^{(n-1)/2}\exp \left( - \frac{\tau}{2} \boldsymbol{\beta}^\top \mathbf{Q} \boldsymbol{\beta}\right)\exp\left\{ - \frac{1}{2} (\boldsymbol{z}/\boldsymbol{n} - \boldsymbol{\beta})^\top \mathrm{diag}(\boldsymbol{n})(\boldsymbol{z}/\boldsymbol{n} - \boldsymbol{\beta})\right\} \tau^{a-1}\exp(-\tau b)
...
walk Metropolis (on the log scale), then drawing from the full
conditional \(\boldsymbol{\beta} \mid \boldsymbol{z}, \tau^{\star}\) and
accepting/rejecting the whole move. A second alternative is to ditch
altogether the data augmentation step and write the unnormalized log
posterior for \(\boldsymbol{\beta}\) as \begin{align*}
...
\FunctionTok{t}\NormalTok{(}\FunctionTok{apply}\NormalTok{(beta, }\DecValTok{2}\NormalTok{, quantile, }\AttributeTok{probs =} \FunctionTok{c}\NormalTok{(}\FloatTok{0.25}\NormalTok{,}\FloatTok{0.5}\NormalTok{,}\FloatTok{0.75}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
                                        25%         50%      75%
(Intercept)                       0.6121975  2.36319510 3.873564
politideo1                       -1.9892041 -0.36613522 1.214107
chefdax1                         -0.9257785  0.68721129 2.283937
brandaction1                     -1.8418958  0.45062649 2.614413
brandaction2                     -2.3650410 -0.14217132 2.051471
politideo1:chefdax1              -1.9118528 -0.42173097 1.146488
politideo1:brandaction1          -2.1911059  0.07257575 2.098037
politideo1:brandaction2          -2.1859102  0.13941346 2.133720
chefdax1:brandaction1            -2.1679751 -0.15518192 2.264913
chefdax1:brandaction2            -2.2688732  0.23541703 2.393560
politideo1:chefdax1:brandaction1 -2.6725076  0.13564511 2.118977
politideo1:chefdax1:brandaction2 -2.1815279  0.12941015 2.497072
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Standard dev. for beta (from Student{-}t)}
\NormalTok{se }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{((nu\_0 }\SpecialCharTok{+}\NormalTok{ n)}\SpecialCharTok{/}\NormalTok{(nu\_0 }\SpecialCharTok{+}\NormalTok{ n }\SpecialCharTok{{-}} \DecValTok{2}\NormalTok{) }\SpecialCharTok{*}\FunctionTok{diag}\NormalTok{(tau\_n}\SpecialCharTok{/}\NormalTok{(nu\_0 }\SpecialCharTok{+}\NormalTok{ n) }\SpecialCharTok{*}\NormalTok{ Omega\_n\_inv))}
