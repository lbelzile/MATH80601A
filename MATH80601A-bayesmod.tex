...
and were last compiled on Monday, January 20 2025.
...
Consider \(Y \sim \mathsf{Poisson}(\lambda)\) with mass function
\begin{align*}
f(y; p)=\exp \left\{ - \lambda + y \log \lambda - \log \Gamma(x+1)\right\}.
\end{align*} and so the canonical parameter is \(Q_1(p) = \log \lambda\)
...
\(\boldsymbol{c}= \mathbf{C}^{-a}(\mathbf{A}\boldsymbol{a} + \mathbf{B}\boldsymbol{b})\).
...
\pandocbounded{\includegraphics[keepaspectratio]{introduction_files/figure-pdf/fig-fundamental-theorem-1.pdf}}
...
\chapter{Monte Carlo methods}\label{monte-carlo-methods-1}
...
conditional distribution, sampling \(p(X_t \mid X_{t-1})\).
...
Lemma 7.9 of
(\citeproc{ref-Robert.Casella:2004}{\textbf{Robert.Casella:2004?}})
shows that the probability of acceptance of a move for the Markov chain
is at least \(1/C\), which is larger than the accept-reject.
\end{remark}
...
see Dyk and Meng (\citeproc{ref-vanDyk.Meng:2001}{2001}) and
(\citeproc{ref-Hobert:2011}{\textbf{Hobert:2011?}}).
...
\textbf{deviance} information criterion of
(\citeproc{ref-Spiegelhalter:2002}{\textbf{Spiegelhalter:2002?}}). It is
...
procedure; see
(\citeproc{ref-Spiegelhalther:2014}{\textbf{Spiegelhalther:2014?}}) and
...
and 4.51: this suggests an improvement in using individual-specific
effects.
