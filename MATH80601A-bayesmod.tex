...
and were last compiled on Monday, March 31 2025.
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, rightrule=.15mm, titlerule=0mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, arc=.35mm, colframe=quarto-callout-important-color-frame, bottomtitle=1mm, toptitle=1mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{\textbf{Learning objectives}:}, opacitybacktitle=0.6, breakable, bottomrule=.15mm, leftrule=.75mm, left=2mm, opacityback=0, coltitle=black]
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, rightrule=.15mm, titlerule=0mm, toprule=.15mm, colbacktitle=quarto-callout-important-color!10!white, arc=.35mm, colframe=quarto-callout-important-color-frame, bottomtitle=1mm, toptitle=1mm, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{\textbf{Summary}}, opacitybacktitle=0.6, breakable, bottomrule=.15mm, leftrule=.75mm, left=2mm, opacityback=0, coltitle=black]
...
\(\psi(\boldsymbol{\theta})\) around \(\theta_{(i)},\) which gives
\begin{align*}
 \psi(\theta) \approx \psi(\theta_{((i)}) + \nabla_{\boldsymbol{\theta}} \psi(\boldsymbol{\theta}_{(i)})(\theta-\theta_{((i)}) + (\theta-\theta_{((i)})^\top\mathbf{H}(\theta_{((i)})(\theta-\theta_{((i)})
\end{align*} The term \(\psi(\theta_{((i)})\) is constant, so if we
plug-in this inside the exponential, we obtain \begin{align*}
 q_{(i+1)}(\boldsymbol{\theta}) &\propto \exp \left\{ - \frac{1}{2} \boldsymbol{\theta}^\top\mathbf{H}(\theta_{(i)}) \boldsymbol{\theta} + \theta_{(i+1)}^\top\mathbf{H}(\theta_{(i)})\boldsymbol{\theta}\right\}
 \end{align*} where the mean of the approximation is \begin{align*}
 \theta_{(i+1)} = \theta_{(i)} - \mathbf{H}^{-1}(\theta_{(i)}) \nabla_{\boldsymbol{\theta}} \psi(\boldsymbol{\theta}_{(i)}).
\end{align*} The iterations perform gradient descent, with a correction
that ajusts for the curvature locally. This scheme works provided that
\(\mathbf{H}(\theta_{(i)})\) is positive definite (all of it's
eigenvalues are positive); this may fail for non-convex targets, in
...
a neighborhood of the mode. The new mean vector \(\theta_{(i+1)}\)
corresponds to a Newton update, and at the same time we have defined a
sequence of Gaussian updating approximations. The fixed point to which
the algorithm converges is the Laplace approximation.

Suppose for simplicity that the domain
\(\boldsymbol{\Theta}=\mathbb{R}^p\), so that no prior transformation
\(T\) is necessary. For location-scale family, we have seen in the
section on ADVI that the variational Bayes with a Gaussian approximation
on the target
...
Gaussian approximation returns a vector \(\boldsymbol{mu}\) around which
the expected value of the gradient is zero and similarly
...
Given the mean and variance of the cavity, denoted \(\mu_{-j}\) and
\(\boldsymbol{\Sigma}_{-j},\) and with
\(s_j = \mathbf{x}_j\boldsymbol{\Sigma}_{-j}\mathbf{x}_j^\top\), the
updated mean and variance are \begin{align*}
\mu_j^* &= s_j^{-1}\boldsymbol{\Sigma}_{-j}\mathbf{x}^\top \{\mathbf{E}(Z_j) -\mathbf{x}_j\mu_{-j}\} \\
\boldsymbol{\Sigma}_{j} &=s_j^{-2}\boldsymbol{\Sigma}_{-j}\mathbf{x}^\top_j\left\{\mathsf{Var}(Z_j) - s_j\right\}\mathbf{x}_j\boldsymbol{\Sigma}_{-j}
...
random variable. The updates to the parameters are found in page 23 of
Cseke and Heskes (\citeproc{ref-Cseke.Heskes:2011}{2011}).
...
\NormalTok{  va }\OtherTok{\textless{}{-}} \FunctionTok{pcubature}\NormalTok{(}\AttributeTok{f =} \ControlFlowTok{function}\NormalTok{(x)\{}\FunctionTok{fn}\NormalTok{(x)}\SpecialCharTok{*}\NormalTok{(x}\SpecialCharTok{{-}}\NormalTok{mu)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{\}, }\SpecialCharTok{{-}}\ConstantTok{Inf}\NormalTok{, }\ConstantTok{Inf}\NormalTok{)}\SpecialCharTok{$}\NormalTok{integral}\SpecialCharTok{/}\NormalTok{cst}
  \CommentTok{\# Calculate canonical parameters}
  \FunctionTok{list}\NormalTok{(}\AttributeTok{Q =} \DecValTok{1}\SpecialCharTok{/}\NormalTok{va, }\AttributeTok{r =}\NormalTok{ mu}\SpecialCharTok{/}\NormalTok{va)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
