...
The Bayesian paradigm is an inferential framework that is widely used in data science. 
<!--It's popularity increased dramatically thanks to the Markov chain Monte Carlo in the 1990's, which allowed for numerical estimation of models that were. 
-->
...
For the posterior to be **proper**, we need the product of the prior and the likelihood on the right hand side to be integrable as a function of $\boldsymbol{\theta}$ over the parameter domain $\boldsymbol{\Theta}.$ The integral in the denominator, $p(\boldsymbol{y}) = \mathsf{E}_{\boldsymbol{\theta}}\{p(\boldsymbol{y} \mid \boldsymbol{\theta})\},$, is termed marginal likelihood or prior predictive distribution: it represents the distribution of the data before data collection, the respective weights being governed by the prior probability of different parameters values. The denominator of @eq-posterior is a normalizing constant, making the posterior density integrate to unity. The marginal likelihood plays a central role in Bayesian testing and needs to be estimated should one try to calculate moments of the posterior via analytical approximations. If $\boldsymbol{\theta}$ is low dimensional, numerical integration such as quadrature methods can be used to compute the marginal likelihood. 
...
From @def-beta, the posterior mean is
 \begin{align*}
...
#| eval: true
#| echo: true
# Log of marginal posterior for binomial with beta prior (default is uniform)
log_marg_post_beta <- function(n, y, alpha = 1, beta = 1){
  lchoose(n, y) + lbeta(alpha + y, beta + n - y) - lbeta(alpha, beta)
}
# Log of Bayes factor
logBF2vs3 <- function(y, n){ 
  # model 2 (beta(1.5,1.5) vs 3 (point mass at 0.5)
  log_marg_post_beta(n = n, y = y, alpha = 1.5, beta = 1.5) - 
    dbinom(x = y, size = n, prob = 0.5, log = TRUE)
}
```
...
@Duke.Amir:2023 consider the difference between integrated and sequential format for sales. The `sellingformat` dataset contains $n=397$ observations split into two groups: quantity-integrated decision ($X_0$ decide the amount to buy) and quantity-sequential ($X=1$, first select buy, then select the amount). Participants of the study were randomly allocated to either of these two format and their decision, either buy, `Y=1`, or do not buy `Y=0`, is recorded. 
...
**Bayes factor**: consider a simple model comparison where the null hypothesis is that the selling format has no impact on the decision to buy or not (no treatment effect). We write $y=\sum_{i=1}^n y_i$ and $y_1 = \sum_{i=1}^n x_iy_i,$ $n_1 = \sum_{i=1}^n x_i$ and $y_0+y_1=y$, $n_0+n_1=n.$ The subscript indicates the groups $0$ (quantity-integrated) and $1$ (quantity-sequential)$.
```{r}
#| eval: true
#| echo: false
...
BF1 <- log_marg_post_bern(n = 198, y = 46) + 
  log_marg_post_bern(n = 199, y = 23)
BF0 <- log_marg_post_bern(n = 397, y = 69)
# Bayes factor
# exp(BF0 - BF1)
```
...
$$\mathcal{L}_0(\theta_0, \theta_1) = \theta_0^{y_0}(1-\theta_0)^{n_0-y_0}\theta_1^{y_1}(1-\theta_1)^{n_1-y_1},$$ the marginal likelihood is $\mathrm{beta}(y+1, n-y+1)$ (cf. @exm-bayesfactor) and the Bayes factor is $`r round(exp(BF0-BF1),3)`$, indicating strong preference for the alternative model, which suggests that there are differences due to present evidence sequentially as opposed to all at once.



**Posterior summary**: with a beta-binomial model, the posterior for the probability of buying is $\mathsf{beta}(47, 153)$ for quantity-integrated and $\mathsf{beta}(24, 177)$ for quantity-sequential. We can compute the posterior of the odds ratio,
...
\end{align*}
In a simulation context, we generate one new dataset (or observation) for each posterior draw $\boldsymbol{\theta}$ from the posterior $p(\boldsymbol{\theta} \mid  \boldsymbol{y}).$
...
Given the $\mathsf{beta}(a, b)$  posterior with  $a=y + \alpha$ and $b=n-y + \beta,$ the predictive distribution of $Y_{\text{new}}$ for fixed $n_{\text{new}}$ number of trials is beta-binomial with mass function
\begin{align*}
...
\end{align*}
which is minimized when $\widehat{\upsilon}=\mathsf{E}_{\Theta \mid \boldsymbol{Y}}(\theta)$.

All of these point estimators are central tendency measures, but some may be more adequate depending on the setting as they can correspond to potentially different values, as shown in the left-panel of @fig-central-moments. The choice is application specific: for multimodal distributions, the mode is likely a better choice.
...
g2 <- ggplot() + 
  stat_function(fun = function(x){
    we*dgamma(x, shape = shape, rate = rate) + 
      (1-we)*dnorm(x, mean = mu, sd = sigma)},
                xlim = c(-2,10), n = 1001) +
  ...
  geom_vline(xintercept = mixt_median,
             ...
             colour = cols[3]) + 
  geom_label(
    mapping = aes(x = mixt_mode, y = 0.9*height, label = "mode"), 
    fill = "white", label.r = unit(0, "lines"),
    colour = cols[1]) +
  geom_label(
    mapping = aes(x = mixt_mean, y = 0.5*height, label = "mean"),
    fill = "white", label.r = unit(0, "lines"),
    colour = cols[2]) +
  geom_label(
    mapping = aes(x = mixt_median, y = 0.1*height, label = "median"),
    fill = "white", label.r = unit(0, "lines"),
    colour = cols[3]) +
  scale_y_continuous(expand = c(0,0)) + 
...
The log likelihood for the full model for $y_i>u$ is
\begin{align*}
...
\end{align*}
We obtained, using tools that will be discussed in @exm-rust, a matrix `post_samp` that contains exact samples from the posterior distribution of $(\tau, \xi, \zeta_u).$ 
To obtain the posterior distribution of the $\alpha$ quantile, $q_{\alpha},$ it thus suffices to plug in each posterior sample and evaluate the function: the uncertainty is carried over from the simulated values of the parameters to those of the quantile $q_{\alpha}.$ The left panel of @fig-lossfn shows the posterior density estimate of the $\mathsf{VaR}(0.99)$ along with the maximum a posteriori (mode) of the latter.
...
Given the posterior distribution of the parameter(s), different loss functions thus lead to different point estimates or summaries to be reported. The quantile or pinball loss is an example that allows user to report selected quantiles, or build regression models to 
...
g3 <- ggplot()  +
  ...
  stat_function(fun = dbeta, 
                ...
                xlim = c(0,1),n = 1001) +
  geom_hline(
    yintercept = dbeta(qbeta(p = 0.75, shape, shape),shape, shape),
    alpha = 0.5, 
    linetype = "dashed") +
  labs(x = "", y = "") +
...
* Under weak conditions on the prior, the large-sample behaviour of Bayesian and frequentist are equal and the contribution of the likelihood part dominates.
...
* The posterior predictive distribution is used for model assessment and prediction. It has a higher variance than the data generating distribution from the likelihood, due to parameter uncertainty.
